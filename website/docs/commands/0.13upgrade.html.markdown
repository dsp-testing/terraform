...
The `terraform 0.13upgrade` command gathers provider source configuration to
help prepare a module that was written for Terraform v0.12 to be used with
Terraform v0.13.
...
providers where possible, and update the configuration to record this
information in a [`required_providers` block][required-providers]. If a
`required_providers` block already exists in one file, that file will be
rewritten. Otherwise, a `versions.tf` file will be created or updated.
